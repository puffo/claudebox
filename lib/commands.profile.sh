...
_cmd_profiles() {
  ...
  local core_profiles="core build-tools shell"
  local language_profiles="c go java javascript php python ruby rust"
  local specialized_profiles="database datascience devops embedded ml networking openwrt security web"

  # Helper function to display profile with status
  display_profile() {
    local profile="$1"
  ...
  # Show Language Profiles
  cecho "Language Profiles" "$DIM"
  ...
  # Show Specialized Profiles
  cecho "Specialized Profiles" "$DIM"
  for profile in $specialized_profiles; do
    if profile_exists "$profile"; then
      display_profile "$profile"
    fi
  done
